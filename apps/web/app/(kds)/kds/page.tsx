...
import {
  Loader2, LogOut, Flame, Clock, CheckCircle2, AlertTriangle, AlertCircle,
...
export default function KdsPage() {
  ...
  // Fetch Pending KDS Items
  const fetchKdsItems = useCallback(async () => {
    try {
      const res = await fetch(`${getApiUrl()}/api/kds/pending?station=kitchen`, {
        credentials: "include",
      })

      if (!res.ok) {
        const data = await res.json()
        throw new Error(data.error || "No se pudieron obtener los pedidos para cocina.")
      }
      ...
      console.error(err)
      setError(err.message || "Error al cargar la comandera de cocina.")
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const handleMarkOrderReady = async (group: OrderGroup) => {
    ...
    try {
      ...
      console.error(err)
      setError("Error al marcar pedido completo como listo.")
    } finally {
      setLoading(false)
    }
  }
  ...
  if (sessionPending || loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-950 text-white">
        ...
        <p className="text-slate-400 text-sm">Cargando Monitor KDS de Cocina...</p>
      </div>
    )
  }
      ...
      <div className="max-w-7xl mx-auto z-10 relative space-y-6">
        ...
        <header className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 pb-4 border-b border-slate-800">
          <div>
            <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight flex items-center gap-3">
              ...
              Monitor <span className="bg-linear-to-r from-amber-400 to-red-400 bg-clip-text text-transparent font-black">KDS Cocina</span>
            </h1>
            <p className="text-slate-400 text-xs mt-1">
              Visualizador marcha de platos en tiempo real — {groupedOrders.length} {groupedOrders.length === 1 ? "pedido en marcha" : "pedidos en marcha"}
            </p>
          </div>
        ...
        {groupedOrders.length === 0 ? (
          <div className="py-24 text-center border border-dashed border-slate-800 rounded-3xl bg-slate-900/20 max-w-xl mx-auto space-y-3">
            <Utensils className="mx-auto h-12 w-12 text-slate-600 mb-1" />
            <h3 className="text-xl font-bold text-slate-300">No hay marchas pendientes en Cocina</h3>
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              Cuando el mozo envíe comanda desde el POS, las comandas de cocina aparecerán automáticamente en esta pantalla.
            </p>
          </div>
        ) : (
                    ...
                    <CardHeader className="p-3.5 border-b border-slate-800/80 pb-3">
                      <div className="flex items-center justify-between">
                        <CardTitle className="text-lg font-black flex items-center gap-1.5">
                          Mesa <span className="text-amber-400 font-black text-xl">#{group.tableNumber}</span>
                        </CardTitle>
                          ...
                          >
                            <div className="flex items-start justify-between gap-2">
                              <div className="flex items-start gap-2">
                                <span className="inline-flex items-center justify-center bg-amber-500/20 text-amber-300 border border-amber-500/40 px-2 py-0.5 rounded-md font-black text-sm min-w-7 text-center">
                                  {item.quantity}x
                                </span>
                                <div>
                            ...
                            {item.notes && (
                              <div className="inline-flex items-center gap-1.5 px-2 py-1 rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-300 text-xs italic font-medium w-fit">
