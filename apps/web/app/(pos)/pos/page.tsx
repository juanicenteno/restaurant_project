...
import { Loader2, LogOut, ShoppingBag, MapPin, Mail, User, Shield } from "lucide-react"
  ...
  if (isPending) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-950 text-white">
        ...
        <p className="text-slate-400 text-sm">Cargando Terminal POS...</p>
      </div>
    )
  }
        ...
        <header className="flex justify-between items-center mb-8 pb-6 border-b border-slate-800">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight">
              Terminal <span className="bg-linear-to-r from-emerald-400 to-teal-400 bg-clip-text text-transparent font-black">POS / Salón</span>
            </h1>
            <p className="text-slate-400 text-sm mt-1">Punto de venta y toma de pedidos</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="md:col-span-2 border-slate-800 bg-slate-900/40 backdrop-blur-md text-white">
            <CardHeader>
              <CardTitle className="text-xl flex items-center gap-2">
                ...
                Información de la Sesión de Servicio
              </CardTitle>
              <CardDescription className="text-slate-400">
                Detalles del camarero/cajero autenticado
              </CardDescription>
            </CardHeader>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="bg-slate-950/40 p-4 rounded-lg border border-slate-800/60">
                  <div className="flex items-center gap-2 text-slate-400 text-xs mb-1">
                    ...
                    <span>ROL DEL OPERADOR</span>
                  </div>
                  <p className="font-semibold text-emerald-400 capitalize">{session.user.role}</p>
                </div>
          ...
          <Card className="border-slate-800 bg-slate-900/40 backdrop-blur-md text-white flex flex-col justify-between">
            ...
            <CardContent className="space-y-2">
              {(session.user.role === "owner" || session.user.role === "manager" || session.user.role === "cashier" || session.user.role === "waiter" || session.user.role === "host") && (
                <Button onClick={() => router.push("/map")} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-semibold">
                  Ver Mapa de Mesas (Salón)
                </Button>
              )}

              {(session.user.role === "owner" || session.user.role === "manager") && (
                <Button onClick={() => router.push("/admin")} className="w-full bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700">
                  Ir a Administración
                </Button>
              )}
